...
<head>
    <meta charset="utf-8">
    <title>Tuntutan disahkan bendahari</title>
</head>
<body style="font-family: Arial, sans-serif; color: #1f2937; line-height: 1.6; margin: 0; padding: 24px; background: #f9fafb;">
    <div style="max-width: 640px; margin: 0 auto; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 8px; padding: 24px;">
        ...
        <table role="presentation" style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            ...
            <tr><td style="padding: 8px 0; color: #6b7280;">Kategori</td><td style="padding: 8px 0;">{{ $claim->category }}</td></tr>
            <tr><td style="padding: 8px 0; color: #6b7280;">Tarikh Tuntutan</td><td style="padding: 8px 0;">{{ $claim->claim_date->format('d/m/Y') }}</td></tr>
        </table>
        @if($claim->treasurer_notes)
            <p style="margin-bottom: 8px;"><strong>Catatan bendahari:</strong></p>
            <p style="white-space: pre-line; background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 6px; padding: 12px;">{{ $claim->treasurer_notes }}</p>
        @endif
        <p><a href="{{ route('claims.show', $claim) }}" style="display: inline-block; background: #dc2626; color: #ffffff; text-decoration: none; padding: 10px 16px; border-radius: 6px;">Semak Tuntutan</a></p>
        <p style="font-size: 13px; color: #6b7280; margin-bottom: 0;">Emel ini dihantar secara automatik apabila tuntutan disahkan di Polistaff.</p>
    </div>
</body>
</html>
